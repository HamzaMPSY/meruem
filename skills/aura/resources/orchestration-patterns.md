...
```
Architect(gpt-5.5/local) ─┐
Skeptic(gpt-5.5/local)   ─┼→ Council 집계 → 최종 결정
Pragmatist(gpt-5.5/local) ─┤      (flags + 가중치)
Tester(gpt-5.5/local)    ─┘
```

**언제**: 중요한 기술 결정, 상충되는 옵션 평가.
- 각 Reviewer가 독립적으로 평가 (격리 로컬 서브에이전트)
- 모든 Council 서브에이전트는 `gpt-5.5`를 사용한다.
- 외부 council 도구, 원격 provider, web research를 사용하지 않는다.
- 각 Reviewer는 `role`, `flag`, `recommendation`, `risks`, `reasoning`을 반환한다.
- `flag`는 `green`, `yellow`, `red` 중 하나만 사용한다.
...
```
병렬 평가:
  Architect(gpt-5.5): 세션 기반 vs 토큰 기반 — 장기 구조 평가 → flag
  Skeptic(gpt-5.5): 세션 기반 vs 토큰 기반 — 실패 모드 평가 → flag
  Pragmatist(gpt-5.5): 세션 기반 vs 토큰 기반 — 구현 비용 평가 → flag
  Tester(gpt-5.5): 세션 기반 vs 토큰 기반 — 검증 전략 평가 → flag

종합자(gpt-5.5):
  → 각 Reviewer의 flag와 평가 취합
